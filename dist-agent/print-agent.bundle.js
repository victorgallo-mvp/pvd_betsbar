  ...
  "../../node_modules/.pnpm/write-file-queue@0.0.1/node_modules/write-file-queue/index.js"(exports2, module2) {
    ...
    WriteQueue.prototype.process = function() {
      ...
      doWhile(function(next) {
        var writeReq = self.queue[0], path = writeReq[0], string = writeReq[1], callback = writeReq[2], index = writeReq[4];
...
var require_custom = __commonJS({
  "../../node_modules/.pnpm/node-thermal-printer@4.6.0/node_modules/node-thermal-printer/lib/types/custom.js"(exports2, module2) {
    ...
    var Custom = class extends PrinterType {
      constructor() {
        super();
        this.config = require_custom_config();
      }
      // ------------------------------ Get paper status ------------------------------
      getStatus() {
        return this.config.TRANSMIT_PAPER_STATUS;
...
function winSharePath(printerName) {
...
async function printViaWindows(printerName, buf) {
  ...
  const sharePath = winSharePath(printerName);
  console.log(`[agent] USB: copy /b "${tmpFile}" "${sharePath}"`);
  return new Promise((resolve, reject) => {
    const proc = (0, import_node_child_process.spawn)("cmd.exe", ["/c", "copy", "/b", tmpFile, sharePath]);
    let output = "";
...
async function markJob(jobId, action) {
...
async function poll() {
  ...
  try {
    ...
    console.error("[agent] Erro ao buscar jobs:", err.message);
    return;
  }
  for (const job of jobs) {
    try {
      ...
      console.error(`[agent] \u2717 Job ${job.id} falhou:`, err.message);
    }
  }
}
